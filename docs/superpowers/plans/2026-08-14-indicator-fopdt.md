...
**Interfaces:**
- Consumes: instrument tag, raw numeric value, `t_sim`, optional profile override.
- Produces: `IndicatorDynamics.profile()`, `.sample()`, `.describe()`, and `.reset()`.

- [x] **Step 1: Write failing profile and response tests**
...
- [x] **Step 3: Implement profile classification and exact FOPDT sampling**
...
**Interfaces:**
- Consumes: `window.IndicatorDynamics`, `lastState.t_sim`, overlay tag/bind records.
- Produces: delayed/lagged displayed values and τ/θ tooltips on both rendering paths.

- [x] **Step 1: Add failing source-integration and coverage assertions**
...
- [x] **Step 2: Run test and verify integration assertions fail**

Run: `node frontend/test_indicator_dynamics.js`

Expected: FAIL because neither render path calls the service.

- [x] **Step 3: Wire both render paths and tooltip metadata**
...
- [x] **Step 1: Document equation, numerical update, parameter matrix, scope, and sources**

Record that the layer is measurement-only and does not replace equipment mass/energy holdup dynamics.

- [x] **Step 2: Remove any closed indicator-dynamics gap and retain unrelated open gaps**
...
- [x] **Step 1: Run frontend tests and full backend regression**
...
Actual regression route: the aggregate Pytest command executed the 30 scenario checks successfully,
then hit the repository's pre-existing collection-time `sys.exit(0)` in
`backend/test_scenario_consequences.py`. Verification was therefore completed as two explicit runs:
Pytest with that script excluded (112 passed), followed by the scenario script itself (30 passed).

- [x] **Step 2: Update the project graph**
...
- [x] **Step 3: Review status and staged diff**
