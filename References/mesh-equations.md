...
- *Closed Gaps:*
  ...
  - Reactor Bubble Point vs. N/C Ratio: The reactor temperature profile is now explicitly constrained to the isobaric bubble point curve. Excess NH3 (high N/C) acts as a volatile diluent, depressing the boiling point. This shift is physically coupled to the upstream Damkohler exotherm anchor (`hpcc["T_prod"]`), propagating the N/C bubble point depression dynamically up the reactor.

### D. Scenario Models
- **HP Stripper Tube Flooding (Scenario C):** 
  The flooded fraction of the tubes is coupled to the sump level above 100%. $flood\_tube\_frac = \max(level_{pct} - 100, 0) / 400.0$. When $flood\_tube\_frac > 0$, heat transfer shifts to pool boiling, decreasing the bottom temperature ($\Delta T_{flood} = -15.0 \times flood\_tube\_frac$) and suppressing stripping efficiency ($g_{flood} = \max(1.0 - \dots - flood\_tube\_frac, \eta_{floor})$).
- **Passivation Air Equilibrium (Scenario D):** 
  Passivation air ($O_2$, $N_2$) entering with the $CO_2$ compressor provides an inert load that affects the partial pressures in the synthesis loop. If tripped, the air vanishes from the stream, condensing $NH_3$ and $CO_2$ more aggressively, dropping synthesis loop pressure, and shifting the inert-purge vent valve composition toward volatile NH3 slip.
