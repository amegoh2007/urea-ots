...
20. **Gap-closure / analysis modules (2026-07-31 research-pass execution) — OFF-ENGINE, self-validated.** Five standalone modules were added under `backend/`, each validated on its own (<1 s, no `main.py` import) before any wiring, exactly as `props_nh3co2h2o.py` / `thermo_extended_uniquac.py` are staged. **They do not change the live equation set in Sections 1–6**; they close/advance the tracked gaps and provide services the later wiring will consume. See `handoff.md` for the per-gap status.
    - **`gap_g6_static_catalogue.py` (G6 catalogue half — CLOSED).** Parses (does not transcribe) the strict PFD source into a static design catalogue of all numbered rows — 264 unique streams, 161 with mass-% composition — each tagged `status=static, resolved=False`, written to `reports/G6_static_stream_catalogue.{json,md}`, kept separate from the live registry. Composition rows confirmed MASS % via the stream-208 mean-MW cross-check ($1/\sum(w_i/M_i)=32.7$ vs tabulated 32.71).
    - **`gap_g6_h0_enthalpy.py` (G6 enthalpy — H0 tier delivered).** Absolute stream enthalpy on ONE elements-at-298.15 K datum, so it is reaction-consistent: $H_{stream}=\sum_i n_i h_i(T)+H^E$ with $h_i=\Delta_fH^\circ+\int_{298.15}^{T}\!C_p\,dT'$. Only $H^E$ is G1-gated; the formation+sensible+phase part is published now. Declared `EnthalpyBasis` flag H0/H1/H2. Datum checks: $h_{H_2O(l)}(25)=\Delta_fH^\circ$; $h_{H_2O(g)}-h_{H_2O(l)}=44.0$ kJ/mol; urea fusion $=13.9$ kJ/mol (Tischer 2019). Aqueous species reuse `props_nh3co2h2o.h0` so the two modules share one datum.
    - **`gap_g9a_ejector_envelope.py` (G9a — advanced).** First-principles primary-nozzle throat from CHOKED motive flow, $\dot m=G^\ast A_t$, $G^\ast=P_0\sqrt{\gamma/(RT_0)}\,(2/(\gamma+1))^{(\gamma+1)/2(\gamma-1)}$, giving $d_t=14.9/26.3/10.1$ mm for 324F002/F004/F005 (primary-exit Mach matches the datasheet "Mach 3–4"); design entrainment, off-design pull curve (Munday–Bagster/Huang double-choke $\omega=(A_3/A_t-A_{py}/A_t)(G_s^\ast/G_p^\ast)$), and a molecular-weight entrainment response. All three duty points close mass on the strict PFD (motive+suction=discharge). Finding: the fitted $A_3/A_t\approx4.4$–$20.6$ straddles/exceeds Huang's refrigeration band (6.44–10.64), so that band does **not** bound these deep-vacuum steam ejectors. Not yet wired into the 324 ejector ODE (documented follow-on).
    - **`gap_g2_reference_state_audit.py` (G2 — re-diagnosed).** Falsifies the research-pass reference-state hypothesis: `thermo_extended_uniquac` solves a water-vapour-only VLE ($a_w\,P^{sat}_w=P$), so urea's standard state never enters (root reconstructed from $\gamma_w P^{sat}$ alone). Confirms urea fusion 132.7 °C from $\Delta_{fus}H/\Delta_{fus}S$. Bounds the off-design slope with an independent Voskov-2012 Margules model (agrees in sign with the sim's UNIQUAC at both stages). Rejects the `Urea-Water VLE Data Research.md` "Fahmy-Nassar" correlation (its pure-water $P^{sat}$ is 22 % below IAPWS-IF97).
    - **`gap_g4_conservation_harness.py` (G4 — proof battery).** Atom-balance, null-feed and Jacobian-sparsity tests for any node (fast self-test PASSES: urea/biuret couples atom-neutral, created-matter caught, pinned-outlet fails the Jacobian test). The `--engine` suite (imports `main.py`, ~13 min) is the gate for retiring `REACT_TEAR_DES` via the equation-oriented reactor solve.
...
**This change was REVERTED**, because it drove $w^{urea}_{D002}$ to 76.515 % against the PFD
stream-317 anchor of 80.00, failing four design-point tests.

**Retraction.** That was first written up as *"the 323 balance misses 80.00 by 3.5 points, and the
pin has been masking it"*. It is false, and the cause was the patch itself. 323D002 Comp-I has one
inlet, one outlet, no reaction and no vapour, so at steady state it must equal its inlet — and that
inlet, $w_{f010}$, measures **80.0014 %**, on the anchor. Comp-I turns over only

$$\alpha = \frac{\dot m\,\Delta t}{M} = \frac{(92\,749/3600)\times 0.25}{67\,600} = 9.5\times 10^{-5}\ \text{per tick}$$

and the patch measured its deviation against a reference captured *once*, then fed the result back
into the state that produced it:

$$w_n = (A-\mathrm{ref}) + w_{n-1}(1-\alpha) + \alpha\,w_{f010}
\qquad\Longrightarrow\qquad w^{\ast} = \frac{A-\mathrm{ref}}{\alpha} + w_{f010}$$

Any constant inside that loop is amplified by $1/\alpha \approx 10\,495$. Replaying the recursion
with a capture error of 0.0003 percentage points converges to 76.5150 % — the observed value to
four decimals.

**The ripple break is real and still open; only the explanation was wrong.** The amplification is
the genuine constraint on any fix, and it eliminates every additive or multiplicative in-loop
correction. Two forms survive: a non-recursive assignment from an upstream variable the pin does
not touch, $w_{auth}=A+(w_{f010}-w_{f010,des})$ — stable and bit-exact, but the tank then tracks
its inlet with no lag — or dropping the pin entirely, which restores both the dynamics and the
44-minute holdup lag but leaves $w_{D002}$ following whatever $w_{f010}$ does. Carried as
**TD-013**.
...
324E001 and 324E003 carried the same identity and are fixed the same way — **TD-015** — but that
half could not be deferred, because the 323D002 strength pin was an *accidental clamp* on it. Pinned,
`urea1_in` is constant and $v_{1,conc}$ sits within 0.4 kg/h of $v_{1,duty}$, so the `min()` tie gave
TIC-324001 partial feedback and the melt drifted only −0.0011 pp/h. Unpinned, the branches separate
by ~74 kg/h and the melt walks at ≈ 0.5 pp/h. TD-013 and TD-015 are one change.

Closing it required a **retune**, because $K_c = 2.0$ / $T_i = 120$ s was inherited from a plant
whose temperature ODE was identically zero and therefore described nothing. The clean measurement is
a central difference over 1 h means, ±0.05 bar on the master in MAN so the plant's own wander
cancels:

| loop | base | +step | −step | $K_p$ |
|---|---|---|---|---|
| TIC-324001 | 130.0033 | 130.4231 | 129.5916 | **+8.32 °C/bar** |
| TIC-324002 | 140.0169 | 140.4353 | 139.6000 | **+8.35 °C/bar** |

Positive on both, so the REVERSE action was right all along — and $K_c = 2.0$ meant a loop gain of
**16.7**, which is exactly the multi-hour limit cycle that was measured ($T_{e003}$ ±1.2 °C,
PV-329212 swinging 81–90 %). Lambda-tuned on the separator's own dynamics, $\tau \approx 360$ s
(180 s residence plus the 180 s bubble-point holdup lag), $\lambda = 3\tau$, $\theta \approx 0$:
...
### 22.6 Level is a volume measurement, so density has to be live

323D002's level spans were **mass** spans built on a frozen 1300 kg/m3. A level gauge measures
volume, so a tank of thinner (hotter or weaker) liquor read low by exactly the density error while
the operator saw the same inventory. The steel volumes do not move; what a kilogram occupies does:

$$L\,[\%] = \frac{M/\rho(w,T)}{V_{full}}\times 100,\qquad
\rho(w,T) = \rho_{PFD} + \big[\rho_{raw}(w,T)-\rho_{raw}(w_{des},T_{des})\big]$$

LIC-323507, LI-323504, the weir-spill threshold and the tie-in pooling all run on it, anchored on
the PFD's 1151 kg/m3 so the design level is bit-exact. At design the active volume comes out at
exactly the licensor's **8.00 m3** — an independent check that the 10 % setpoint and the 1151 kg/m3
density are consistent with each other and with the < 6 min residence the compartment exists for.

### 22.7 cp is a property, not a section constant
...
### 22.10 Full-audit source closures and ejector evidence boundary

The 328C003 hydrolyser no longer uses a pseudo-first-order conversion. Its live rate coefficient is

$$k_H(T)=\exp\!\left(21.8-\frac{11100}{T_K}\right)\quad\mathrm{m^3\,kmol^{-1}\,h^{-1}},$$

and the second-order PFR integral is evaluated from the live urea and water concentrations. The
reaction heat is explicit at $\Delta H_{hyd}=+101.5$ kJ/mol and is debited from the hydrolyser energy
balance. The correlation is extrapolated above its experimental range to the 200 °C plant state; that
limitation remains visible in the full audit.

Both Unit-324 evaporation stages now close the coupled temperature/pressure/VLE/vapour-load tear by
bounded fixed-point iteration inside each dynamic tick. The 9-bar steam header includes the PFD
4,933 kg/h effective flash-recovery source (`6687 - 1754`) and the live 324E003 heat demand. Dynamic
recycle tears publish residual vectors, tolerance, and convergence state; algebraic tear classification
and accelerated iteration remain open.

The three issue-for-order ejector sheets establish two separate, mass-conservative evidence cases:
324F002 gives `94 + 650 = 744 kg/h`; the F004/F005 package gives, at maximum load,
`761 + 600 - 1300 + 505 = 566 kg/h`, closing against `535 + 31.5 = 566.5 kg/h` to tabular rounding.
The strict PFD instead gives `72 + 390 = 462 kg/h` for F002 and an exact 584/1220/41/180/190/31
kg/h balance through F004/E006/F005/E007. The active model retains the PFD anchors. The revised
Körting drawings close manufacturer/designation/serial, UAN 01-3042 package provenance, substantial
F004/F005 fabrication geometry, and body operating points of 0.122 bar(a)/77 °C and 0.245 bar(a)/
100 °C. The user confirmed the old page-7 `SUEZ II 01-3040` text is a copied template error.

They still do not contain a complete set of unambiguous effective hydraulic areas/loss coefficients,
the F004/E006 interstage pressure chain, critical backpressure, pull/correction/dryness curves,
breakdown/recovery hysteresis, or acceptance-test points. The proportional pressure/motive pull
