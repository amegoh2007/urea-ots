...
while the header still holds 4 barg bit-exact (the Tsat(P_LP) coupling to the HPCC, hence the plant
...
def test_k207b_bias_passes_the_design_export() -> None:
    # At the anchored bias opening and design pressure differential, PV-329207B passes exactly
...
def test_design_fixed_point_holds_with_turbine_export() -> None:
    ...
    assert st.P_LP == pytest.approx(ss.P_LP_BARA, abs=0.02)  # 4 barg, absolute model basis
...
def test_over_pressure_opens_turbine_and_recovers_setpoint() -> None:
    ...
    m0 = st.m_turbine
    st.P_LP = ss.P_LP_BARA + 0.3                         # inject over-pressure
    ss.step_steam(st, DT, ss.M_STRIP_DES, M_GEN_DES)
    assert st.m_turbine > m0                             # biased PI opens the export further
    _settle(st, 3000)
    assert st.P_LP == pytest.approx(ss.P_LP_SP_BARA, abs=0.05)  # recovers to SP
