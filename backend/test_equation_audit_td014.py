...
def test_bubble_point_anchors_are_bit_exact():
    ...
    assert main.evap_w_eq(130.0, 0.33, main.R324_W_EV1, 130.0, 0.33) == main.R324_W_EV1
...
def test_raoult_is_retained_only_for_the_subrange_f010_closure():
    ...
    assert abs(raoult - 99.0) < 8.0
    assert abs(raoult - 99.0) < 0.40 * elev
...
def test_the_unit_324_stages_carry_the_same_closure():
    ...
    strength follows the design-anchored Extended-UNIQUAC neutral-species limit, so the water
    ...
    for tag in ("E001", "E003"):
        ...
        assert d["thermo_model"] == main.extended_uniquac.MODEL_NAME
        assert d["thermo_validity"] == "DESIGN_ANCHORED_EXTRAPOLATION"
        assert "iteration_residual" in d and "converged" in d
